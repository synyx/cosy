// https://codepen.io/jeffibacache/pen/bzBsp?editors=0010
...
(function () {
	...
	document.addEventListener("arcade-closed", (event) => {
	...
	function generateThrustParticle() {
		...
		p.lifeSpan = 80;
		p.pos.setXY(
			ship.pos.getX() + Math.cos(ship.angle) * -14,
			ship.pos.getY() + Math.sin(ship.angle) * -14,
		);
	...
	function generateAsteroidExplosion(asteroid) {
		...
		for (i; i > -1; --i) {
			...
			p.pos.setXY(
				asteroid.pos.getX() +
					Math.cos(p.vel.getAngle()) * (asteroid.radius * 0.8),
				asteroid.pos.getY() +
					Math.sin(p.vel.getAngle()) * (asteroid.radius * 0.8),
			);
	...
	function renderAsteroids() {
		...
		for (i; i > -1; --i) {
			...
			context.moveTo(
				(a.pos.getX() +
					Math.cos(doublePI * (j / a.sides) + a.angle) * a.radius) >>
					0,
				(a.pos.getY() +
					Math.sin(doublePI * (j / a.sides) + a.angle) * a.radius) >>
					0,
			);
